...
contract UniswapV2Pair is IUniswapV2Pair, UniswapV2ERC20 {
    ...
    // reserve0 和 reserve1 則是代表 Pair 中還剩下多少，簡單來說 reserve0 就是 balanceOf(token0)。有相關的 Getter Function 。
    ...
    function mint(address to) external lock returns (uint liquidity) {
        // 在白皮书中提到，保存缓存余额是为了防止攻击者操控价格预言机。此处还用于计算协议手续费，并通过当前余额与缓存余额相减获得转账的代币数量。
        ...
        // 否则根据转入的代币价值与当前流动性价值比例铸造流动性代币。
        if (_totalSupply == 0) {
            // liquidity 代表將要被鑄造給 Liquidity Provider 的 LP Token 數量。
            // x 幣數量 * y 幣數量然後開根號 再減去並銷毀 1000 wei 手續費
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        // 由于在swap方法最后会检查余额（扣掉手续费后）符合k常值函数约束（参考白皮书公式），因此合约可以先将用户希望获得的代币转出，
    ...
    // 參考說明: https://hackmd.io/@adshao/HkZwPZNf9
