// build a bare-bones client that interacts with the oracle
...
async function getCallerContract (web3js) {
  const networkId = await web3js.eth.net.getId() // 指定 network id
  return new web3js.eth.Contract(CallerJSON.abi, CallerJSON.networks[networkId].address) // 回傳實例化 caller contract
}
...
(async () => {
  ...
  setInterval( async () => {
    // 呼叫 caller contract - updateEthPrice function 並指定發送交易的 ownerAddress
